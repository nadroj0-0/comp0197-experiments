...
def ensure_run_snapshot(model_name: str, run_name: str):
    ...
    return run_dir, run_experiment_yml, run_model_yml


def run_search_from_snapshot(model_name: str, run_name: str):
    run_dir, run_experiment_yml, run_model_yml = ensure_run_snapshot(model_name, run_name)
...
def preprocess_from_base_model(self_model, model_name: str,
    ...
    if any(
        value is None
        for value in (self_model.train_raw, self_model.val_raw, self_model.test_raw, self_model.item_weights)
    ):
...
def run_full_pipeline(self_model, model_name: str, run_name: str = None,
    ...
    run_dir, _, run_model_yml = ensure_run_snapshot(model_name, run_name)

    if do_search:
        run_search_from_snapshot(model_name, run_name)
