...
# The only line you should ever edit here is SEARCH below.
#
# Usage:
...
def _load_train_data(exp_train: dict, first_train_cfg: dict,
    ...
    """
    Load all training data loaders once before the training loop.

    Uses full-dataset settings from experiment.yml train block (sampling=all).
    Mirrors the existing three-loader routing pattern exactly:
        det models    → zscore=True  loaders
        prob models   → zscore=False loaders
        NB models     → zscore=False loaders
    When use_normalise=False all three point to the same loader.

    Returns dict with keys:
        train_loader_det/gauss/nb
        val_loader_det/gauss/nb
        test_loader_det/gauss/nb
        stats_det/gauss/nb
    """
    ...
    if not use_norm:
        ...
        tl_det,   vl_det,   tel_det,   s_det   = build_dataloaders(**data_kwargs, zscore_target=True)
        tl_gauss, vl_gauss, tel_gauss, s_gauss = build_dataloaders(**data_kwargs, zscore_target=False)
        tl_nb,    vl_nb,    tel_nb,    s_nb    = build_dataloaders(**data_kwargs, zscore_target=False)
...
def main():
    ...
    # Use the first model's train_config for data field values
    # ------------------------------------------------------------------
    exp_train       = exp_cfg.get("train", {})
    first_model_yml = run_dir / "configs" / "models" / f"{models[0]}.yml"
    first_train_cfg = load_train_config(first_model_yml)
    ...
    for model_name in models:
        ...
        # load train_config from run yml
        # if search ran, best_config is already merged into train_config
