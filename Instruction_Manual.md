...
5. Add your model name to `configs/experiment.yml`
6. Run `python train.py --experiment configs/experiment.yml`
...
Running `train.py`: loads experiment config → builds models via registry → optional hyperparameter search → trains on full dataset → saves everything to `runs/run_name/`. All runs are seeded and configs are snapshotted at start time, so results are fully reproducible.
...
```
Group/
├── train.py                    ← entry point, edit SEARCH flag only
├── search.py                   ← hyperparameter search runner
├── test.py                     ← evaluation and plots
├── configs/
│   ├── experiment.yml          ← controls which models run + data settings
│   ├── registry.yml            ← maps model names to builder functions
│   └── models/
│       └── your_model.yml      ← hyperparameters for your model
└── utils/
...
```bash
# Train all models listed in experiment.yml
python train.py

# Override batch size / workers for GPU
python train.py --batch_size 4096 --num_workers 8

# Point at a different experiment file
python train.py --experiment configs/my_experiment.yml

# Run evaluation and generate plots
python test.py
```

Set `SEARCH = True` at the top of `train.py` to run hyperparameter search before training. Set `SEARCH = False` (default) to train directly using the values in your model yml.
...
When `SEARCH = True`, the pipeline runs successive halving before final training:
...
Every time you run `train.py`, the current state of `experiment.yml` and all referenced model ymls are copied into `runs/run_name/configs/`. This means:
